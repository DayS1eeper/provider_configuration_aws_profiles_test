resource "scalr_workspace" "v3_agent" {
  name              = "v3_agent"
...
resource "scalr_variable" "v3_agent_bucket" {
  key            = "bucket_name"
  value          = var.bucket_name
  category       = "terraform"
  environment_id = scalr_environment.test.id
  workspace_id   = scalr_workspace.v3_agent.id
}

# object names
resource "scalr_variable" "v3_agent_obj_role_delegation" {
  key            = "aws_v3_object_role_delegation"
  value          = var.obj_role_delegation_v3_agent
  category       = "terraform"
  environment_id = scalr_environment.test.id
  workspace_id   = scalr_workspace.v3_agent.id
}

resource "scalr_variable" "v3_agent_obj_access_keys" {
  key            = "aws_v3_object_access_keys"
  value          = var.obj_access_keys_v3_agent
  category       = "terraform"
  environment_id = scalr_environment.test.id
  workspace_id   = scalr_workspace.v3_agent.id
}

resource "scalr_variable" "v3_agent_obj_instance_profile" {
  key            = "aws_v3_object_instance_profile"
  value          = var.obj_instance_profile_v3_agent
  category       = "terraform"
  environment_id = scalr_environment.test.id
  workspace_id   = scalr_workspace.v3_agent.id
}

resource "scalr_variable" "v3_agent_obj_export_shell_vars" {
  key            = "aws_v3_object_export_shell_variables"
  value          = var.obj_export_shell_vars_v3_agent
  category       = "terraform"
  environment_id = scalr_environment.test.id
  workspace_id   = scalr_workspace.v3_agent.id
}
