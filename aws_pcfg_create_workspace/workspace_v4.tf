resource "scalr_workspace" "v4_no_agent" {
  name              = "v4_no_agent"
...
resource "scalr_variable" "v4_no_agent_bucket" {
  key            = "bucket_name"
  value          = var.bucket_name
  category       = "terraform"
  environment_id = scalr_environment.test.id
  workspace_id   = scalr_workspace.v4_no_agent.id
}

# object names
resource "scalr_variable" "v4_no_agent_obj_role_delegation" {
  key            = "aws_v4_object_role_delegation"
  value          = var.obj_role_delegation_v4
  category       = "terraform"
  environment_id = scalr_environment.test.id
  workspace_id   = scalr_workspace.v4_no_agent.id
}

resource "scalr_variable" "v4_no_agent_obj_access_keys" {
  key            = "aws_v4_object_access_keys"
  value          = var.obj_access_keys_v4
  category       = "terraform"
  environment_id = scalr_environment.test.id
  workspace_id   = scalr_workspace.v4_no_agent.id
}

resource "scalr_variable" "v4_no_agent_obj_instance_profile" {
...
resource "scalr_variable" "v4_no_agent_obj_export_shell_vars" {
  key            = "aws_v4_object_export_shell_variables"
  value          = var.obj_export_shell_vars_v4
  category       = "terraform"
  environment_id = scalr_environment.test.id
  workspace_id   = scalr_workspace.v4_no_agent.id
}
...
resource "scalr_variable" "v4_no_agent_create_instance_profile_resource" {
